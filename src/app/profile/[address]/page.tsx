...
import { MEMBER_MANAGER_ADDRESS, MemberABI, LEVEL_NAMES } from '../../lib/constants';
...
export default function UserProfilePage() {
  ...
  // 客户端就绪后更新标题（保留水合兼容逻辑）
  useEffect(() => {
    if (isClientReady && targetAddress && currentUserAddress) {
        ...
        <div className="space-y-6">
          ...
          {isClientReady && targetAddress === currentUserAddress && (
            <div className="flex flex-wrap gap-3">
              <Link
                href="/posts/create"
                className="inline-flex items-center gap-2 px-4 py-2 bg-gradient-to-r from-purple-700 to-indigo-700 text-white rounded-full hover:shadow-lg hover:shadow-purple-700/20 transition"
              >
              ...
              <button
                onClick={() => router.push('/')}
                className="inline-flex items-center gap-2 px-4 py-2 glass-effect border border-border text-[#EAE6F2] rounded-full hover:bg-white/5 transition"
              >
